// Lists functionality - Fixed version with upload fix
...
function showUploadContactsModal(listId, listName) {
    ...
    // IMPORTANT FIX: Remove any existing event listeners
    const uploadBtn = document.getElementById('uploadContactsBtn');
    const newUploadBtn = uploadBtn.cloneNode(true);
    uploadBtn.parentNode.replaceChild(newUploadBtn, uploadBtn);
    
    // Add event listener for upload button
    document.getElementById('uploadContactsBtn').addEventListener('click', uploadContacts);
}

// Upload contacts
async function uploadContacts() {
    try {
        console.log('Upload contacts function called');
