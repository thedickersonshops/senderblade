...
def create_template():
    ...
    try:
        template_id = execute_db(
            'INSERT INTO spinner_templates (name, content, subject, description, signature) VALUES (?, ?, ?, ?, ?)',
            (name, content, subject, description, signature)
        )
...
@spinner_api.route('/spinner/templates/<int:template_id>', methods=['PUT'])
def update_template(template_id):
    ...
    try:
        execute_db(
            'UPDATE spinner_templates SET name = ?, content = ?, subject = ?, description = ?, signature = ? WHERE id = ?',
            (name, content, subject, description, signature, template_id)
        )
        ...
        return jsonify({'success': False, 'message': f'Error updating template: {str(e)}'}), 500

@spinner_api.route('/spinner/templates/<int:template_id>', methods=['DELETE'])
def delete_template(template_id):
    """Delete a template"""
    try:
        execute_db('DELETE FROM spinner_templates WHERE id = ?', (template_id,))
...
@spinner_api.route('/spinner/templates/<int:template_id>', methods=['GET'])
def get_template(template_id):
    """Get a specific template"""
    try:
        template = query_db('SELECT * FROM spinner_templates WHERE id = ?', (template_id,), one=True)
        ...
        return jsonify({
            ...
            'data': {
                ...
                'signature': template_dict.get('signature', ''),
